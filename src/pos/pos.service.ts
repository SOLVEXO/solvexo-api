...
export class PosService {
  ...
  async pinLogin(dto: PinLoginDto) {
    ...
    return {
      ...
      data: { employee: safe, activeSession: activeSession ?? null },
    };
  }
  ...
  async removeShift(sellerId: string, storeId: string, shiftId: string) {
    await this.verifyStoreOwnership(storeId, sellerId);
  ...
  async openSession(sellerId: string, dto: OpenSessionDto) {
    ...
    if (existingOpen) throw new BadRequestException('This register already has an open session');
  ...
  async getActiveSession(storeId: string, registerId: string) {
  ...
  async refundSale(sellerId: string, saleId: string) {
    const sale = await this.r.saleModel.findById(saleId);
    if (!sale) throw new NotFoundException('Sale not found');
    if ((sale as any).status === 'refunded') throw new BadRequestException('Sale is already refunded');
    ...
    await this.r.saleModel.findByIdAndUpdate(saleId, { status: 'refunded' });

    // reverse session totals
    const sessionUpdate: any = { $inc: { totalRefunds: (sale as any).total, totalTransactions: -1 } };
    const pm = (sale as any).paymentMethod;
    ...
    sessionUpdate.$inc.totalSales = -(sale as any).total;

    await this.r.registerSessionModel.findByIdAndUpdate((sale as any).sessionId, sessionUpdate);

    return { success: true, message: 'Sale refunded and stock restored' };
  }
